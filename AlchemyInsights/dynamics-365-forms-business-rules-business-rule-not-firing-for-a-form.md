---
title: Dynamics ๓๖๕กฎทางธุรกิจของฟอร์ม-กฎทางธุรกิจไม่ยิงสำหรับฟอร์ม
ms.author: pebaum
author: pebaum
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom:
- "1467"
- "6200018"
ms.openlocfilehash: 66911e4f87d54f635b397d02fefdcaadb68d0ecc
ms.sourcegitcommit: defe2c412567b596fa8c3ab52111bde712ebb314
ms.translationtype: MT
ms.contentlocale: th-TH
ms.lasthandoff: 10/29/2019
ms.locfileid: "37769394"
---
# <a name="business-rule-not-firing-for-a-form"></a>กฎทางธุรกิจที่ไม่ได้ยิงสำหรับฟอร์ม

กฎทางธุรกิจอาจไม่ดำเนินการเนื่องจากฟิลด์ที่อ้างอิงในกฎทางธุรกิจไม่รวมอยู่ในแบบฟอร์ม สำหรับข้อมูลเพิ่มเติมโปรดดู[ที่กฎทางธุรกิจของคุณจะไม่ถูกสร้างสำหรับฟอร์มหรือไม่](https://docs.microsoft.com/powerapps/maker/model-driven-apps/create-business-rules-recommendations-apply-logic-form#is-your-business-rule-not-firing-for-a-form)
