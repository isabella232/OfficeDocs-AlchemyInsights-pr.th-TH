---
title: ส่งออกที่ติดต่อใน Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "279"
- "8000020"
ms.assetid: 37fc1455-7324-4c76-9a94-085a45e94c51
ms.openlocfilehash: 9f8a3c2ad2b2d6736d34f168bc5733d554ee32e95d8ea6f12069feea1a12ec49
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: th-TH
ms.lasthandoff: 08/05/2021
ms.locfileid: "54024693"
---
# <a name="export-your-contacts-in-outlookcom"></a>ส่งออกที่ติดต่อของคุณOutlook.com

1. ไปที่ [บุคคล](https://outlook.live.com/people/) <img src='data:image/png;base64,' />
.
2. ที่ด้านบนของหน้า ให้เลือก **จัดการ** \> การส่งออก
3. เลือกว่าคุณต้องการส่งออกที่ติดต่อทั้งหมดหรือเฉพาะที่ติดต่อจากโฟลเดอร์ที่ระบุ **แล้วเลือก** ส่งออก
4. ที่ด้านล่างของหน้า ให้เลือก บันทึก และ " **บันทึก** contacts.csv" จะถูกบันทึกในโฟลเดอร์ดาวน์โหลดเริ่มต้นของคุณ เมื่อต้องการเลือกที่ที่จะ.csvไฟล์บันทึก ให้เลือกลูกศรที่อยู่ถัดจาก บันทึก **แล้วเลือก บันทึก** เป็น

For more information, see [Export contacts from Outlook.com to a CSV file](https://support.office.com/article/578cca22-3550-4c73-b3f0-9978cfeac83f?wt.mc_id=Office_Outlook_com_Alchemy).